...
using AccountService.Tests.Extensions;
...
public class TransactionApiTests(CustomWebApplicationFactory factory, ITestOutputHelper output)
{
    [Fact]
    public async Task Parallel_Transaction_MustPreservationTotalBalance()
    {
        // Arrange
        ...
        for(var i = 0; i < 50; i++)
        {
            var httpContent = i % 2 == 0 ? new StringContent(json1, Encoding.UTF8, "application/json") : new StringContent(json2, Encoding.UTF8, "application/json");
        ...
        output.WriteLine($"Account1 transactions count {accountStatement1.Transactions.Length}");
        output.WriteLine($"Account2 transactions count {accountStatement2.Transactions.Length}");
